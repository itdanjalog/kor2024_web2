...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http ) throws Exception {
        ...
        http.authorizeHttpRequests( ( httpReq ) -> {
            ...
            // [3**] 403 에러 메시지는 권한이 없을때 페이지 접근 차단.  // 403 에러 페이지를 핸들러(매핑) 하기.
        http.exceptionHandling( ( e ) -> // 403 에러 핸들러
                e.accessDeniedHandler( ((request, response, accessDeniedException) -> {  response.sendRedirect("/error403");   }) ) // URL 반환한다.
        );
